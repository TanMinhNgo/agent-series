...
export function LibraryPage() {
  ...
  const toggleProjectSource = (item: LibraryAsset) => {
  ...
  const pinAsset = (item: LibraryAsset, projectId: string) => {
    if (projectId) updateAsset.mutate({ id: item.id, data: { projectId, isProjectSource: true } });
    setPinningAssetId(null);
  };
  const renderFileCard = (item: LibraryAsset) => (
    <article key={item.id} className="rounded-xl border p-4">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        {item.mimeType} · {size(item.sizeBytes)} · {new Date(item.createdAt).toLocaleDateString('vi-VN')}
      </p>
      ...
      <div className="mt-4 flex justify-between">
        ...
        <div className="flex gap-1">
          ...
          <Button
            ...
            onClick={() => toggleProjectSource(item)}
      ...
      {pinningAssetId === item.id ? (
        <select
          ...
          defaultValue=""
          onChange={(event) => pinAsset(item, event.target.value)}
        >
  ...
  const renderFiles = () => (
    <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {(assets.data || []).map(renderFileCard)}
      {!assets.data?.length ? (
  ...
  const renderCollection = (collection: NonNullable<typeof collections.data>[number]) => (
    <article key={collection.id} className="rounded-lg border p-3">
      <div className="flex items-center justify-between gap-3">
        <button
          className="text-left font-medium hover:underline"
          onClick={() => setEditingCollectionId(editingCollectionId === collection.id ? null : collection.id)}
        >
          {collection.name}{' '}
      ...
      {editingCollectionId === collection.id ? (
        <div className="mt-3 space-y-2">
          ...
          {!(documents.data || []).some((document) => document.projectId === collectionProjectId) ? (
    ...
    worker.data ? (
      ...
      >
        ...
        <p className="mt-1 text-muted-foreground">
          Queue: {worker.data.queued} chờ · {worker.data.running} đang chạy · {worker.data.failed} thất bại
    ...
    <section className="rounded-xl border p-4">
      ...
      {collectionProjectId ? (
        <>
          ...
          <div className="mt-3 space-y-2">
            {(collections.data || []).map(renderCollection)}
  ...
  const renderDocument = (item: NonNullable<typeof documents.data>[number]) => (
    <article key={item.id} className="rounded-xl border p-4">
  ...
  const renderDocuments = () => (
    <div className="mt-5 space-y-3">
      {renderWorkerStatus()}
      {renderCollections()}
      {(documents.data || []).map(renderDocument)}
      ...
      >
        ...
        >
          ...
          <div className="mt-5 flex justify-end gap-2">
            ...
            <Button
              type="submit"
              disabled={updateAsset.isPending || !assetName.trim() || assetName.trim() === renamingAsset.name}
            >
