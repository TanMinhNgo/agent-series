...
export function useAdminDashboard({
  ...
  const users = useQuery({
    queryKey: ['admin', 'users', query, userPage],
    queryFn: () =>
      request<AdminList<AdminUser>>({
        url: '/admin/users',
  ...
  const credentials = useQuery({
    ...
    queryFn: () =>
      request<AdminList<AdminCredential>>({
  ...
  const updateUser = useMutation({
    mutationFn: ({ userId, isActive }: { userId: string; isActive: boolean }) =>
      request({ url: `/admin/users/${userId}/active`, method: 'PATCH', data: { isActive } }),
    onSuccess: () => {
  ...
  const updateModel = useMutation({
    mutationFn: ({ provider, modelId, isActive }: { provider: string; modelId: string; isActive: boolean }) =>
      request({
        url: `/admin/models/${encodeURIComponent(provider)}/${encodeURIComponent(modelId)}/active`,
        method: 'PATCH',
        data: { isActive },
      }),
    onSuccess: () => {
