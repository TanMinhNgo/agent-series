...
async def _upload_documents(deps: KnowledgeDependencies, files: list[UploadFile], project_id: str | None) -> list[dict[str, Any]]:
    if project_id and deps.services().workspace.get(deps.project_model, project_id) is None:
        raise HTTPException(status_code=422, detail=PROJECT_NOT_FOUND)
    uploaded = []
    try:
        for file in files:
...
def _delete_document(deps: KnowledgeDependencies, document_id: str) -> None:
...
def _document_file(deps: KnowledgeDependencies, document_id: str) -> Response:
...
def _set_collection_documents(deps: KnowledgeDependencies, collection_id: str, payload: CollectionDocumentsRequest) -> dict[str, Any]:
    try:
...
def build_router(deps: KnowledgeDependencies) -> APIRouter:
    ...
    def set_collection_documents(collection_id: str, payload: CollectionDocumentsRequest) -> dict[str, Any]:
        return _set_collection_documents(deps, collection_id, payload)
    ...
    def document_file(document_id: str) -> Response:
        return _document_file(deps, document_id)
    ...
    async def upload_documents(files: list[UploadFile] = File(...), project_id: str | None = Form(default=None)) -> list[dict[str, Any]]:
        return await _upload_documents(deps, files, project_id)
    ...
    def delete_document(document_id: str) -> None:
        _delete_document(deps, document_id)
