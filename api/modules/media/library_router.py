...
async def _upload_assets(deps: LibraryDependencies, files: list[UploadFile], project_id: str | None) -> dict[str, list[dict[str, Any]]]:
    uploaded, errors = [], []
...
def _update_asset(deps: LibraryDependencies, asset_id: str, payload: UpdateArtifactRequest) -> dict[str, Any]:
...
def _asset_file(deps: LibraryDependencies, asset_id: str) -> Response:
...
def _reindex_asset(deps: LibraryDependencies, asset_id: str) -> dict[str, Any]:
...
def _delete_asset(deps: LibraryDependencies, asset_id: str) -> None:
    with deps.services().chats.database.session() as session:
        asset = session.get(deps.library_asset_model, asset_id)
        if asset is None:
...
def _artifact_value(deps: LibraryDependencies, operation: Callable[[str], Any], asset_id: str) -> Any:
    try:
...
async def _create_version(deps: LibraryDependencies, asset_id: str, file: UploadFile) -> dict[str, Any]:
    try:
...
def build_router(deps: LibraryDependencies) -> APIRouter:
    ...
    async def upload_library_assets(files: list[UploadFile] = File(...), project_id: str | None = Form(default=None, alias="projectId")) -> dict[str, list[dict[str, Any]]]:
        return await _upload_assets(deps, files, project_id)
    ...
    def update_library_asset(asset_id: str, payload: UpdateArtifactRequest) -> dict[str, Any]:
        return _update_asset(deps, asset_id, payload)
    ...
    async def create_library_asset_version(asset_id: str, file: UploadFile = File(...)) -> dict[str, Any]:
        return await _create_version(deps, asset_id, file)
    ...
    def restore_library_asset_version(asset_id: str) -> dict[str, Any]:
        item = _artifact_value(deps, deps.services().library.restore_version, asset_id)
        deps.enqueue_artifact_index(item)
        return deps.library_asset_json(item)
    ...
    def preview_library_asset(asset_id: str) -> dict[str, Any]:
        return _artifact_value(deps, deps.services().artifacts.preview, asset_id)
    ...
    def diff_library_asset(asset_id: str) -> dict[str, Any]:
        return _artifact_value(deps, deps.services().artifacts.diff, asset_id)
    ...
    def library_asset_file(asset_id: str) -> Response:
        return _asset_file(deps, asset_id)
    ...
    def reindex_library_asset(asset_id: str) -> dict[str, Any]:
        return _reindex_asset(deps, asset_id)
