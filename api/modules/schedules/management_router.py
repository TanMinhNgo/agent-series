...
def _update_schedule(deps: ScheduleManagementDependencies, schedule_id: str, payload: ScheduleUpdateRequest) -> dict[str, Any]:
    ...
    values = _schedule_update_values(deps, current, payload)
...
def _run_now(deps: ScheduleManagementDependencies, schedule_id: str) -> dict[str, str]:
...
def _resend_email(deps: ScheduleManagementDependencies, schedule_id: str, run_id: str) -> dict[str, Any]:
    schedule = deps.services().workspace.get(deps.schedule_model, schedule_id)
    if schedule is None:
        raise HTTPException(status_code=404, detail=deps.schedule_not_found_error)
...
def build_router(deps: ScheduleManagementDependencies) -> APIRouter:
    ...
    def update_schedule(schedule_id: str, payload: ScheduleUpdateRequest) -> dict[str, Any]:
        return _update_schedule(deps, schedule_id, payload)
    ...
    def run_schedule_now(schedule_id: str) -> dict[str, str]:
        return _run_now(deps, schedule_id)
    ...
    def resend_schedule_run_email(schedule_id: str, run_id: str) -> dict[str, Any]:
        return _resend_email(deps, schedule_id, run_id)
