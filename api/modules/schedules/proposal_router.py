...
def _find_block(deps: ScheduleProposalDependencies, session: Any, chat_id: str, proposal_id: str):
...
def _confirm(deps: ScheduleProposalDependencies, source_chat: Any, session: Any, message: Any, blocks: list, config: dict, proposal_id: str) -> dict[str, Any]:
    ...
    schedule = deps.schedule_model(
        title=proposal.title, prompt=proposal.prompt, starts_at=proposal.starts_at,
        recurrence=proposal.recurrence, timezone=proposal.timezone,
        project_id=config.get("projectId"), provider=source_chat.provider,
        model=source_chat.model, status="active", next_run_at=proposal.starts_at,
    )
...
def _mutate(deps: ScheduleProposalDependencies, chat_id: str, proposal_id: str, action: Literal["confirm", "dismiss"]) -> dict[str, Any]:
    ...
    with deps.services().chats.database.session() as session:
        message, blocks, config = _find_block(deps, session, chat_id, proposal_id)
        if action == "confirm":
            return _confirm(deps, source_chat, session, message, blocks, config, proposal_id)
...
def build_router(deps: ScheduleProposalDependencies) -> APIRouter:
    ...
    def confirm_chat_schedule_proposal(chat_id: str, proposal_id: str) -> dict[str, Any]:
        return _mutate(deps, chat_id, proposal_id, "confirm")
    ...
    def dismiss_chat_schedule_proposal(chat_id: str, proposal_id: str) -> dict[str, Any]:
        return _mutate(deps, chat_id, proposal_id, "dismiss")
